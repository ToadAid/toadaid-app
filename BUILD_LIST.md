...
- **P4B-P18 — COMPLETE:** admit one user-supplied imagined Lore Land composition and three user-supplied real deed captures through a specific repository ceremony; blend the imagined composition into Home; add a responsive, keyboard-accessible World view with local-only public-address validation, real-deed fixture cards, and exact Base/OpenSea source references; ownership observation, metadata retrieval, wallet connection, signing, authentication, persistence, and network activation remain not performed;
- **P4B-P19 — COMPLETE:** establish an imagined Home archipelago with one focal transparent Lore Land cutout, derived locally from the admitted composition, a visual step larger than three depth-scaled transparent lands generated from the user's admitted deed references; cluster the lands around the hero in fixed pixel offsets from the stage center, allow presentation-only click or keyboard activation to swap a side land with the centered hero, lock the wide-viewport Home shell to one screen height with the fixture panel scrolling inside its rail, and retire the procedural Three.js world from the Home runtime so its constructed geometry no longer competes with the artwork, while preserving the specialist fixture label, World behavior, source semantics, and authority-none posture;
- **P4B-P20 — COMPLETE:** cut the World view's public-address observer live as read-only observation: a pasted Base address is resolved against the Canonical Lore Land Deeds contract through a keyless Multicall3 `ownerOf` batch scan on a public Base RPC, cached locally with its observed block, and each held deed is presented OpenSea-style with its gateway-resolved image, name, token id, and the five metadata trait rows; refresh/clear controls and an explicit zero-result state preserve fail-visible behavior; wallet connection, signing, authentication, API keys, persistence beyond the local cache, and any authority interpretation of observed ownership remain not performed and are forbidden by the stage admission;
- **P4B-P21 — COMPLETE:** add an optional in-app WalletConnect wallet connection to the World view as read-only address observation: the user pairs a mobile wallet through a QR modal rendered in-page over a pairing methods allowlist of `eth_accounts`, `eth_requestAccounts`, and `eth_chainId` only — never signing, transactions, or message signatures — and the connected address, chain, and connection time are displayed and persist until explicit disconnect, then feed the existing read-only deed lookup exactly as a pasted address would through the same input path, while fail-closed chain admission refuses non-Base chains, the WalletConnect SDK's own `wc@2:` storage is classified as relay-transport-only material that disconnect removes, and the connected address carries no authentication, membership, PrincipalId, or authority so Stage D remains blocked;
- **P4B-P22 — COMPLETE:** polish the World view deed viewer with three presentation-only capabilities on the already-admitted P20 endpoints: resolved live cards open a per-deed detail modal (artwork, description, traits, observed owner, observed block, OpenSea/Tobyworld links) that populates only from a policy-validated fail-closed record rendered through `textContent` alone, with Escape/backdrop/Close dismissal, Tab trapping, focus restore, and a `setView` close so navigation never strands it; a direct id lookup observes any single deed id 1–6000 straight from the Base contract through one plain `ownerOf` call with strict parsing, fixed rejection messages, no network request on malformed input, honest revert-shaped unminted copy, and a degraded modal when metadata fails — all without ever touching the block-stamped deed-index cache; and the gallery gains fixed-aspect shimmer skeletons and fail-visible error cards whose run-token-guarded Retries are refused when superseded, while fixture cards stay static, no new network origin or persistence is admitted, and presentation remains authority-none so Stage D stays blocked;
